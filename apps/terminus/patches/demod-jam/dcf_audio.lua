...
local function decode(w)
	...
	if (w[2] >> 4) ~= VERSION then
		return nil, "bad version"
	end
	if crc16(w, 15) ~= ((w[16] << 8) | w[17]) then
		return nil, "crc"
	end
	return {
		ftype = w[2] & 0xF,
...
function M.packetize(codec_id, payload, packet_id, ts_us, src, dst, flags)
	...
	for k = 1, frag_total do
		local off = (k - 1) * 4
		local chunk = { payload[off + 1] or 0, payload[off + 2] or 0, payload[off + 3] or 0, payload[off + 4] or 0 }
...
function Reassembler:push(frame)
	...
	if not d or d.ftype ~= M.FCTRL then
		return nil
	end
...
function Reassembler:finalize()
	local lost = {}
	for pid in pairs(self.slots) do
		lost[#lost + 1] = pid
	end
	table.sort(lost)
	self.slots = {}
...
local function selfcert()
	local a = {}
	for i = 1, 9 do
		a[i] = string.byte("123456789", i)
	end
	if crc16(a, 9) ~= 0x29B1 then
		return false
	end
	-- the channel hash reuses the certified crc16, so the same anchor pins it:
	if M.channel_from_passphrase("123456789") ~= 0x29B1 then
		return false
	end

	local frames = M.packetize(1, { 0xDE, 0xAD, 0xBE, 0xEF, 0x12, 0x34 }, 0x10, 0x010203, 0x0001, 0xFFFF, 0)
	local want = {
	...
	for i = 1, 3 do
		if frame_to_hex(frames[i]) ~= want[i] then
			return false
		end
	end
	...
	local got
	for i = 1, #frames do
		local p = r:push(frames[i])
		if p then
			got = p
		end
	end
	if not got or got.codec_id ~= 1 or #got.payload ~= 6 then
		return false
	end
	if got.payload[1] ~= 0xDE or got.payload[6] ~= 0x34 then
		return false
	end

	-- channel filter: accept on match/broadcast, reject otherwise
	if not M.accepts(0x1234, 0x1234) or not M.accepts(M.BROADCAST, 0x1234) then
		return false
	end
	if M.accepts(0x1234, 0x5678) then
		return false
	end

	local pp = { f0 = 0x1234, amp = 17, mod_index = 99, mod_ratio = 5, bright = 33, env = 7, flags = 1 }
	local up = M.pm_unpack(M.pm_pack(pp))
	if up.f0 ~= pp.f0 or up.flags ~= pp.flags or up.mod_index ~= pp.mod_index then
		return false
	end

 ...
 *
 * CAS-based ownership tracking triple buffer. A single atomic uint32_t
 * encodes which buffer each side owns. Writer and reader each CAS to
 * swap their buffer with the "middle" buffer. No torn reads possible.
 *
 * All operations are wait-free. Suitable for shared memory (shm_open).
 */
...
typedef struct DemodParamSnapshot {
    ...
    float synth_gain;          /* Applied to summed instrument buses */
...
/* ── State Word ────────────────────────────────────────────────────
 *   bits [1:0]  writer index    bits [3:2]  middle index
 *   bits [5:4]  reader index    bit  [6]    dirty flag
 * Invariant: writer, middle, reader are a permutation of {0,1,2}.
 * ──────────────────────────────────────────────────────────────── */
...
/* Producer: get exclusively-owned write buffer */
...
/* Producer: swap writer ↔ middle, set dirty */
...
/* Consumer: if dirty, swap reader ↔ middle, clear dirty. Returns owned buffer. */
...
/* Consumer: safe copy into caller memory */
